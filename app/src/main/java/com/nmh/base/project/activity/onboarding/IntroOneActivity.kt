...
import com.nmh.base.project.databinding.ActivityIntroOneBinding
...
class IntroOneActivity: BaseActivity<ActivityIntroOneBinding>(ActivityIntroOneBinding::inflate) {

    override fun setUp() {
        FirebaseAnalytics.getInstance(this).logEvent("v_ui_intro1_${BuildConfig.VERSION_CODE}", Bundle())
        showNative()

        AdsConfig.loadNativeIntro2(this@IntroOneActivity)
        AdsConfig.loadInterIntro1(this@IntroOneActivity)
    ...
    private fun showInterIntro() {
        if (haveNetworkConnection() && ConsentHelper.getInstance(this).canRequestAds()
            && AdsConfig.interIntro1 != null && AdsConfig.is_load_inter_intro1) {
            Admob.getInstance().showInterAds(this, AdsConfig.interIntro1, object : AdCallback() {
                override fun onNextAction() {
                    super.onNextAction()
                    startIntent(IntroTwoActivity::class.java.name, true)
                }

                override fun onAdClosedByUser() {
                    super.onAdClosedByUser()
                    AdsConfig.interIntro1 = null
                    AdsConfig.loadInterIntro2(this@IntroOneActivity)
                    AdsConfig.lastTimeShowInter = System.currentTimeMillis()
                }
            })
        } else startIntent(IntroTwoActivity::class.java.name, true)
    }

    private fun showNative() {
        if (haveNetworkConnection() && AdsConfig.is_load_native_intro1
            && ConsentHelper.getInstance(this).canRequestAds()) {
            binding.layoutNative.visible()
            AdsConfig.nativeIntro1?.let {
                pushViewAds(it)
            } ?: run {
                Admob.getInstance().loadNativeAd(this, getString(R.string.native_intro1),
