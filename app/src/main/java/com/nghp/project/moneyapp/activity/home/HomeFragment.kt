...
import com.nghp.project.moneyapp.activity.search.SearchActivity
...
import com.nghp.project.moneyapp.databinding.FragmentHomeBinding
        ...
        transactionAdapter.callback = object : ICallBackItem {
            override fun callBack(ob: Any?, position: Int) {
                if (ob is TransactionItem.Transaction) {
                    startIntent(
                        Intent(requireContext(), DetailTransactionActivity::class.java).apply {
                    ...
                    val list = state.data.filterIsInstance<TransactionItem.Transaction>()
                        .filter {
                            ...
                            val formattedItemDate =
                                itemDate?.let { it1 -> targetFormat.format(it1) }
                        ...
                        when (typeModel) {
                            TypeModel.INCOME, TypeModel.EXPENSES -> {
                                ...
                                binding.tvTotal.text =
                                    "$currency${FormatNumber.convertNumberToNumberDotString(total)}"
                            }
                            ...
                            TypeModel.LOAN -> {
                                ...
                                binding.tvTotalLoan.text = "$currency${
                                    FormatNumber.convertNumberToNumberDotString(totalLoan)
                                }"
                                binding.tvTotalBorrow.text = "$currency${
                                    FormatNumber.convertNumberToNumberDotString(totalBorrow)
                                }"
                            }
                        }
